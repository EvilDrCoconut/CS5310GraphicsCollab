  ...
  if (ppmFile.is_open()){
      ...
      while ( getline (ppmFile,line) ){
        ...
        if (line[0]=='#'){
			...
			// done: Can be stored optionally	
		 }else{
             // if the ppm file has more than one entries in a single line
             if (line.length() > 10){
                char *num = strtok((char*)line.c_str(), " ");
                for (int j = 0; j < m_width*3; j++) { 
                    m_PixelData[pos] = (unsigned char)atoi(num);
                    ++pos;
                    num = strtok(NULL, " ");
                }
             } else {
                // if the ppm file has single entry per line
