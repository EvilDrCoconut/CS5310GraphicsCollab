...
// a helper function used to process keyboard press events
void doKeyDown(SDL_KeyboardEvent *event);

// a variable to help toggle between wire and texture modes
bool wireMode;

// return bool value for wire mode. True means wire mode will be on, else 
// texture mode
bool getWireMode(){
	return wireMode;
}

// a helper function used to set bool value for wire mode. 
// True means wire mode will be on, else texture mode
...
void SDLGraphicsProgram::update(float x, float y, float z){
    ...
    house->getTransform().translate(0.0f,0.0f+y,-8.0f);
    ...
    // planet->getTransform().loadIdentity();
    // planet->getTransform().translate(-3.0f,1.0f,-8.0f+z);
    // planet->getTransform().rotate(rot,0.0f,1.0f,0.0f);
    // planet->getTransform().scale(0.5f,0.5f,0.5f);
    // planet->update(screenWidth,screenHeight);
...
void SDLGraphicsProgram::render(){
    ...
    (*house).render(); // Equivalent to house->render();
    (*chapel).render();
    (*windmill).render();
    // (*planet).render();
...
void doKeyDown(SDL_KeyboardEvent *event)
{
	// a bool value to handle wiremode and texture mode with same key (w) pressed
	// value true means wire mode, false means texture mode
		bool isWireMode = getWireMode();

		// wire mode
		if (event->keysym.scancode == SDL_SCANCODE_W && isWireMode==true)
		{
		...
		// texture mode 	
		if (event->keysym.scancode == SDL_SCANCODE_W && isWireMode==false)
		{
